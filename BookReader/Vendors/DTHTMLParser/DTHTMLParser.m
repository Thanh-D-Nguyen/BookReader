...
- (void)_resetAccumulateBufferAndReportCharacters
{
    ...
    NSInteger len = [_accumulateBuffer length];
    NSRange range = NSMakeRange(_charCount, len);
    [self.delegate parser:self foundCharacters:_accumulateBuffer range:range];
    _charCount += len;
	// reset buffer
...
- (void)setDelegate:(id <DTHTMLParserDelegate>)delegate
{
    ...
    if ([delegate respondsToSelector:@selector(parser:foundCharacters:range:)])
	{
